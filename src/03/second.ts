...
export function solve(input: string[]): number {
  const gearLocations = findGearLocations(input)
  const partNumberLocations = findPartNumberLocations(input)
  const map = createMap(input)

  const gears = gearLocations.reduce((value, location) => {
    findNeighboringPartNumbers(map, location.coord).forEach(coord => {
      ...
      if (partNumber) {
        if (!value.has(location.coord)) {
          value.set(location.coord, new Set())
        }

        value.get(location.coord)?.add(Number(partNumber.value))
      }
    })

    return value
  }, new JsonKeyMap<Coord, Set<number>>())

  return gears.values()
    .filter(set => set.size === 2)
    .map(set => Array.from(set.values()).reduce((product, partNumber) => product * partNumber, 1))
...
export function findGearLocations(lines: string[]): EngineLocation[] {
  return findEngineLocations(/[\\*]/g, lines)
}
