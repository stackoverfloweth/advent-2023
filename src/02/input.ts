...
export type Reveal = { color: Color, count: number }
export type Reveals = Reveal[]
export type Game = { id: number, reveals: Reveals[] }

export function parseInput(value: string): Game[] {
  return value.split('\n').map(mapGame)
}

export function mapGame(value: string): Game {
  const gameRegex = /Game (\d+):(.*)/g
  const [matches] = [...value.matchAll(gameRegex)]
  const [, id, rest] = matches

  return {
    id: toNumber(id),
    reveals: rest.split(';').map(mapReveals),
  }
}

export function mapReveals(value: string): Reveals {
  return value.split(',').map(mapReveal)
}

export function mapReveal(value: string): Reveal {
  const revealRegex = /(\d+) (red|blue|green)/g
  const [matches] = [...value.matchAll(revealRegex)]
...
export function toNumber(value: string): number {
  ...
  if (isNaN(parsed)) {
    throw `invalid number found! ${value}`
  }

  return parsed
}

export function isGame(value: Game | Reveals): value is Game {
  return 'id' in value
}
